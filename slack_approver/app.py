...
device flow), after enforcing:
  ...
  - approver is not the PR author (segregation of duties)
...
def approve_modal(ack, body, view, client):
    ...
    if login.lower() == author.lower():
        return refuse("you authored this PR. Approver must differ from author.", approver=login)
    ...
    ack()
    audit(action="approve", slack_user=slack_user, approver=login, author=author, repo=repo,
          pr=number, sha=card_sha, risk=risk, result="approved", review_id=r.json()["id"])
